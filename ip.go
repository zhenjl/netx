...
// AddIPs adds a list of IP addresses. Each of the IPs in the list are parsed and
// expanded using ParseIP.
func (this *iplist) AddIPs(ips []string) error {
	this.mu.Lock()
	defer this.mu.Unlock()

	if this.ipm == nil {
		this.ipm = make(map[[4]byte]struct{})
	}

	for _, ip := range ips {
		iplist, err := ParseIP(ip)
		if err != nil {
			return err
		}
...
// ParseIP takes a string that represents an IP address, IP range, or CIDR block and
